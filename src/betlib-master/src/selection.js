...
export class WinSelection extends Selection {
  constructor({ placeOddsFraction = "1/1", rule4 = 0, winOdds }) {
    super();

    this.winOdds = winOdds;
    this.rule4 = rule4;

    const [num, denom] = placeOddsFraction.split("/");
    const decimalPlaceOddsFraction = parseInt(num) / parseInt(denom);
    this.placeOdds = 1 + (this.winOdds - 1) * decimalPlaceOddsFraction;

    if (this.rule4 < 0 || this.rule4 > 0.9) {
...
export class DeadHeatSelection extends WinSelection {
  constructor({ placeOddsFraction = "1/1", placesOffered, rule4 = 0, runnersInDeadHeat, tiedPosition, winOdds }) {
    super({ placeOddsFraction, rule4, winOdds });
