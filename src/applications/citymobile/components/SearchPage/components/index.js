...
import { getFeaturedLeagues } from "../../../../citydesktop/components/Navigation/LeftNavigation/utils/NavigationUtils";
...
const SearchPage = () => {
  ...
  const clearAllRecent = () => {
    localStorage.removeItem("recentEvents"); // legacy - this can be removed after Sep 2022
      ...
      <div className={classes["search__wrapper"]}>
        <div className={classes["search__search"]}>
          <div className={classes["search__input"]}>
            ...
            />
            <span
              className={`${classes["search__close"]} ${searchString.length > 0 ? classes["active"] : ""}`}
              onClick={() => setSearchString("")}
            />
          </div>
          <div className={classes["search__cancel"]}>
        ...
        <div className={`${classes["search-result"]} ${showSearchResults ? classes["active"] : ""}`}>
          <h2 className={classes["search-result__title"]}>
            ...
            {!searchLoading && searchCouponData ? `(${searchEvents.length})` : ""}
          </h2>
              ...
              if (match.path["League"].toUpperCase().includes(searchString.toUpperCase())) {
                return (
                  <div
                    ...
                    key={match.id}
                    onClick={() => onSelectEventPathHandler(match.code, match.parent.substr(1, match.parent.length))}
                  >
        ...
        <div className={`${classes["search-recommended"]} ${!showSearchResults ? classes["active"] : ""}`}>
          {searchString.length > 2 ? (
            <div className={classes["search-recent"]}>{t("city.navigation_search.no_search_results")}</div>
          ) : null}
          ...
          <h2 className={classes["search-recommended__title"]}>{t("city.navigation_search.recommended_events")}</h2>
