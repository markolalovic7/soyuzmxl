...
import MobileStakePanel from "../../BetslipModal/components/MyBetslip/components/MobileStakePanel";

import DesktopStakePanel from "./DesktopStakePanel";
...
const MyBetslipFooter = ({ activeTab, setPlaceBetTime, setStakePanelStatus, stakePanelStatus }) => {
  ...
  const isUnder1080 = currentWidth < 1080;
      ...
      {stakePanelStatus.open &&
        (isUnder1080 ? (
          ...
          <DesktopStakePanel
            ...
            warningShown={
              anyPriceChanged ||
              betslipData.model.outcomes.findIndex((outcome) => outcome.marketId && !outcome.valid) !== -1
            }
            onClose={() => setStakePanelStatus({ open: false })}
          />
        ))}
      {(!isUnder1080 || !stakePanelStatus.open) && (
